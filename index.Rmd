...
#from lowercase to uppercase
raw_claims_feb21 <- mutate_each(raw_claims_feb21, funs(toupper))

names(raw_claims_feb21 )
```

```{r}
nrow(clean_claims_21)

clean_claims_21$clean_claims_21 <- 1
```
...
```{r}
#Merge claims 21 with codified claims

claims_21 <- merge(x = raw_claims_feb21_ch, y = clean_claims_21_ch, by = c("expediente", "fur", "numero_hec", "numero_fud"), all.x = TRUE)
